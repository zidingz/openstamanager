...
?><form action="" method="post" id="edit-form">
    ...
    if (sizeof($rs_doc) > 0) {
        if (sizeof($rs_doc) == 1) {
            ...
            <div class=" col-md-2">
                <br>
                <a href="<?php echo ROOTDIR; ?>/editor.php?id_module=<?php echo module($modulo)['id']; ?>&id_record=<?php echo $rs_doc[0]['iddocumento']; ?>" class="btn btn-info"><i class="fa fa-chevron-left"></i> <?php echo tr('Vai alla fattura'); ?></a>
            </div>
        <?php
        } else {
            ?>
        ...
        <?php
            for ($i = 0; $i < sizeof($rs_doc); ++$i) {
                ...
                $modulo = ($rs[0]['dir'] == 'entrata') ? 'Fatture di vendita' : 'Fatture di acquisto'; ?>
                        <li><a href="<?php echo ROOTDIR; ?>/editor.php?id_module=<?php echo module($modulo)['id']; ?>&id_record=<?php echo $rs_doc[$i]['iddocumento']; ?>" class="dropdown-item"><?php echo tr('Vai alla fattura n. '.$rs_doc[$i]['numero']); ?></a></li>
        <?php
            } ?>
                    </ul>
                </div>

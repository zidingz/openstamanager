...
function add_tecnico($idintervento, $idtecnico, $inizio, $fine, $idcontratto = null)
{
    ...
    $info = $dbo->fetchOne('SELECT idanagrafica, idsede_destinazione, id_tipo_intervento FROM in_interventi WHERE id='.prepare($idintervento));
    $anagrafica = Modules\Anagrafiche\Anagrafica::find($info['idanagrafica']);
    $sede = Modules\Anagrafiche\Sede::find($info['idsede_destinazione']) ?: $anagrafica->sedeLegale;
...
function aggiungi_intervento_in_fattura($id_intervento, $id_fattura, $descrizione, $id_iva, $id_conto, $id_rivalsa_inps = false, $id_ritenuta_acconto = false, $calcolo_ritenuta_acconto = false)
{
    ...
    $dbo->query("UPDATE in_interventi SET id_stato=(SELECT id FROM in_statiintervento WHERE descrizione='Fatturato') WHERE id=".prepare($id_intervento));
}

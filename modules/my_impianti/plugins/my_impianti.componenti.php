...
if (!empty($rs2)) {
    ...
    for ($j = 0; $j < $n2; ++$j) {
        ...
        $interventi = $dbo->fetchArray('SELECT *, DATE_FORMAT(data_richiesta,"%d/%m/%Y") AS data_richiesta, (SELECT descrizione FROM in_tipiintervento WHERE id=in_interventi.idtipointervento) AS tipo, (SELECT descrizione FROM in_statiintervento WHERE id=in_interventi.idstatointervento) AS stato, (SELECT colore FROM in_statiintervento WHERE id=in_interventi.idstatointervento) AS colore FROM in_interventi INNER JOIN my_componenti_interventi ON my_componenti_interventi.id_intervento=in_interventi.id WHERE id_componente='.prepare($rs2[$j]['id']).' ORDER BY id_intervento');
        if ($interventi != null) {

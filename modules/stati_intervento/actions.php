...
switch (post('op')) {
    case 'update':
        ...
        // Aggiorna
        $query = 'UPDATE in_statiintervento SET descrizione='.prepare($descrizione).',  colore='.prepare($colore).', completato='.prepare($completato).' WHERE id='.prepare($id_record);
        $dbo->query($query);
    ...
    case 'add':
        ...
        if (count($dbo->fetchArray('SELECT id FROM in_statiintervento WHERE id='.prepare($idstatointervento))) > 0) {
            $_SESSION['errors'][] = tr('Stato di intervento già esistente.');
        } else {
            $query = 'INSERT INTO in_statiintervento(id, descrizione, colore) VALUES ('.prepare($idstatointervento).', '.prepare($descrizione).', '.prepare($colore).')';
            $dbo->query($query);
    ...
    case 'delete':
        ...
        if (count($dbo->fetchArray('SELECT id FROM in_interventi WHERE idstatointervento='.prepare($id_record))) > 0) {
            $query = 'UPDATE in_statiintervento SET deleted = 1 WHERE id='.prepare($id_record).' AND `can_delete`=1';
        } else {

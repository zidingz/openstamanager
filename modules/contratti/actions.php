...
switch (post('op')) {
    ...
    case 'update':
        if (post('id_record') !== null) {
            ...
            $contratto->data_bozza = post('data_bozza', true);
            $contratto->data_accettazione = post('data_accettazione', true);
            $contratto->data_rifiuto = post('data_rifiuto', true);
            $contratto->data_conclusione = post('data_conclusione', true);
            ...
            foreach (post('costo_ore') as $id_tipo => $valore) {
                ...
                if (sizeof($rs) == 0) {
                    ...
                    if (post('costo_ore')[$id_tipo] != 0 || post('costo_km')[$id_tipo] != 0 || post('costo_dirittochiamata')[$id_tipo] != 0) {
                        $dbo->query('INSERT INTO co_contratti_tipiintervento(idcontratto, id_tipo_intervento, costo_ore, costo_km, costo_dirittochiamata, costo_ore_tecnico, costo_km_tecnico, costo_dirittochiamata_tecnico) VALUES('.prepare($id_record).', '.prepare($id_tipo).', '.prepare(post('costo_ore')[$id_tipo]).', '.prepare(post('costo_km')[$id_tipo]).', '.prepare(post('costo_dirittochiamata')[$id_tipo]).', '.prepare(post('costo_ore_tecnico')[$id_tipo]).', '.prepare(post('costo_km_tecnico')[$id_tipo]).', '.prepare(post('costo_dirittochiamata_tecnico')[$id_tipo]).')');
                    }
                }

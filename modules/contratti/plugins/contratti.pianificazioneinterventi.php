    ...
    case 'delete-promemoria':
     ...
     case 'pianificazione':
            ...
            if (!empty($idcontratto_riga) && !empty($intervallo)) {
                ...
                '(SELECT descrizione FROM in_tipiintervento WHERE id=co_righe_contratti.idtipointervento) AS tipointervento FROM co_righe_contratti '.
...
// Righe già inserite
$qp = 'SELECT *, (SELECT descrizione FROM in_tipiintervento WHERE id=co_righe_contratti.idtipointervento) AS tipointervento FROM co_righe_contratti WHERE idcontratto='.prepare($id_record).' ORDER BY data_richiesta ASC';
$rsp = $dbo->fetchArray($qp);
            ...
            <table class="table table-condensed table-striped table-hover">
                ...
                <tbody>
                    <tr>
                        ...
                        <td>
                            {[ "type": "select", "placeholder": "'.tr('Tipo intervento').'", "name": "idtipointervento", "required": 1, "values": "query=SELECT id, descrizione FROM in_tipiintervento ORDER BY descrizione ASC", "value": "'.$rsp[0]['idtipointervento'].'" ]}
                        </td>
                        <td>

...
if (isset($id_record)) {
    ...
    $record = $dbo->fetchOne('SELECT *,
       ...
       (SELECT is_fatturabile FROM co_staticontratti WHERE id=id_stato) AS is_fatturabile,
       (SELECT is_pianificabile FROM co_staticontratti WHERE id=id_stato) AS is_pianificabile,
       (SELECT is_completato FROM co_staticontratti WHERE id=id_stato) AS is_completato,
